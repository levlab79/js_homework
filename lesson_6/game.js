...
const game = {
  ...
  start() {
    ...
    const questionNumber = this.makeStep(firstQuestion);
    console.log(questionNumber);

    // Получаем ID следующего вопроса.
    // for (let question in nodes[]){
    //   if 
    // }
    // return;
...
const quest = {
  ...
  start() {
    while (true) {
      ...
      const userAnswer = +prompt(`${this.nowQuestion["text"]}\n${this.getOptions()}`);
      // Если 0, то выходим из игры.
      if (userAnswer === 0) {
        alert('Спасибо за игру!')
        return;
      } else {
        // Определяем индекс следующего вопроса.
        this.nowId = this.nowQuestion["options"][userAnswer - 1]["nextText"];
      }
    }
  },
  ...
  getOptions() {
    let options = '';
    for (let index in this.nowQuestion["options"]) {
      options += String(Number(index) + 1) + '. ' + this.nowQuestion["options"][index]["text"] + '\n';
    }
    return options;
  },
  ...
  getQuestion() {
    ...
    for (let questionId in nodes) {
      if (nodes[questionId]["id"] === this.nowId) {
        this.nowQuestion = nodes[questionId];
        break;
      }
    }
  }
